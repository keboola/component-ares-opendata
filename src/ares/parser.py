...
def process_data(source_file, output_folder):
    with tarfile.open(source_file, 'r:gz') as tf, open(
            os.path.join(output_folder, 'firmy.csv'), 'w', encoding='utf8') as ud, \
            open(os.path.join(output_folder, 'fosoby.csv'), 'w', encoding='utf8') as fo, \
            open(os.path.join(output_folder, 'posoby.csv'), 'w', encoding='utf8') as po:
        udc = csv.writer(ud)
        foc = csv.writer(fo)
        poc = csv.writer(po)
        ...
        for rw, el in enumerate(tf):
            ...
            vypisy = odp[0].findall('.//are:Vypis_VREO', namespaces=et.nsmap)
            for vypis in vypisy:
                dt = []
                ch = [j.tag for j in vypis.getchildren()]
                ch = [j[j.rindex('}') + 1:] for j in ch]

                sekce = {'Uvod', 'Zakladni_udaje', 'Statutarni_organ', 'Jiny_organ'}
                ...
                dt.extend(attr(uvod, uvod_cols, et))
                ...
                zakl_cols = [
                    'Rejstrik', 'ICO', 'ObchodniFirma', 'DatumZapisu',
                    'DatumVymazu'
                ]
                zi = attr(zakl, zakl_cols, et)
